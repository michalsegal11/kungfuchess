...
from core.engine.events import MovePlayed, PieceTaken, ErrorPlayed, JumpPlayed
...
class Piece:
    ...
    def on_command(self, cmd: Command, now_ms: int, game=None) -> bool:
        ...
        if cmd.type == "Move":
            ...
            if self._is_path_blocked(src_cell, dst_cell, game):
                _dbg(f"[{self.piece_id}] blocked mid-path {src_cell}->{dst_cell}")
                if game:
                    game.bus.publish(ErrorPlayed(now_ms, "blocked mid-path", self.piece_id))
                return False

            if self._is_ally_on_cell(dst_cell, game):
                _dbg(f"[{self.piece_id}] ally occupies {dst_cell}")
                if game:
                    game.bus.publish(ErrorPlayed(now_ms, "ally on target", self.piece_id))
                return False
    ...
    def _is_path_blocked(self,
        ...
        if not game or self.can_jump_over_allies:
            return False                        # can transit over allies

        dr = dst[0] - src[0]
        dc = dst[1] - src[1]
        steps = max(abs(dr), abs(dc))
        if steps <= 1:
            return False                        # short step
        ...
        blocked_cells = {
            ...
            if p is not self and not p.is_captured
        }
    ...
    # -------------------------------------------------------------
    #  Helpers – ally-occupy check
    # -------------------------------------------------------------
    def _is_ally_on_cell(self,
                         cell: tuple[int, int],
                         game) -> bool:
        """
        Check if an ally piece occupies the specified cell.
        Args:
        ...
        my_is_white = self.piece_id[1] == "W"
        for p in game.pieces:
            if p.is_captured or p is self:
                continue
            if p.current_state.physics.get_current_cell() != cell:
                continue
            if (p.piece_id[1] == "W") == my_is_white:     # אותו צבע
                return True
        return False
