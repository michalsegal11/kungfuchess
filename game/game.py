...
from game.move_history  import record_move
from ui import ui
...
from engine.events  import EventBus, GameStarted, GameEnded,PieceTaken,ErrorPlayed

from graphics.img   import Img
from pathlib        import Path
import cv2
from game.constants import PIECE_VALUE
...
class Game:
    ...
    def __init__(self, pieces: List[Piece], board: Board):
        ...
        self.game_start_ms: int | None = None
        ...
        self.move_log_ui = MoveLogUI(self.bus)
        self.score_ui    = ScoreUI(self.bus)
        self.overlay     = Overlay(self.bus)
        self.sound_fx    = SoundFX(self.bus, "snd/move.wav", "snd/capture.wav", "snd/jump.wav", "snd/error.wav")
    ...
    def _resolve_collisions(self):
        """Check for collisions on same cells, resolve by time precedence."""
        collisions = {}
        ...
        for pieces_in_cell in collisions.values():
            ...
            if len(colors) == 1:
                continue
            pieces_in_cell.sort(key=lambda p: p.current_state.physics.start_time_ms, reverse=True)
            ...
            for loser in pieces_in_cell[1:]:
                if (winner in self.white_pieces and loser in self.black_pieces) or \
                   (winner in self.black_pieces and loser in self.white_pieces):
                    ...
                    from game.move_history import fmt_elapsed
    ...
    def _is_win(self) -> bool:
        """Return True if a king has been captured (after delay)."""
        ...
        for color, king in kings.items():
            if king.is_captured:
                if not hasattr(self, '_win_timer_ms'):
                    ...
                    print(f"{color} king captured – finishing in 2 s…")
                    self.bus.publish(GameEnded(color))
                    return False
